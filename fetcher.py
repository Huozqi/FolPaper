...
class LiteratureFetcher:
    ...
    def fetch_all(self, start_date=None, end_date=None):
        ...
        total_new = 0
        for sub in subs:
            # 优先使用用户自定义的 source_name，如果没有则默认
            source_name = sub.get('source_name')
            if not source_name:
                source_name = f"arXiv: {sub['sub_value']}" if sub['sub_type'] == 'arxiv' else f"RSS: {sub['sub_value']}"

            # 使用订阅配置的抓取天数（如果未设置默认 7 天）
            fetch_days = sub.get('fetch_days') or 7
            current_start_date = end_date - timedelta(days=fetch_days)
                
            if sub['sub_type'] == 'arxiv':
                total_new += self.fetch_arxiv(sub['sub_value'], current_start_date, end_date, max_results=1000, source_name=source_name)
            elif sub['sub_type'] == 'rss':
                total_new += self.fetch_rss(sub['sub_value'], current_start_date, end_date, source_name=source_name)
            elif sub['sub_type'] == 'pubmed':
                from pubmed_service import PubMedService
                pubmed_service = PubMedService()
                # 订阅时拉取条目数可以适当调大以确保不错过，最大可以调大到 200
                results = pubmed_service.search(sub['sub_value'], retmax=200, start_date=current_start_date, end_date=end_date)
                for res in results:
                    res['source'] = source_name
                    # PubMed 的文献可能没有完整规范的 ISO 日期格式，而我们的清理逻辑非常依赖 published 字段的准确性
                    # 在存入数据库之前，确保其能过日期校验
                    if self.db.add_article(res):
                        total_new += 1
            elif sub['sub_type'] == 'openalex':
